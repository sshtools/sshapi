...
public class E20CustomChannel {
	...
	public static void main(String[] arg) throws Exception {
		 ...
		 * the createChannel() method is called. This allows the client to react to
		 * the initial request data and configure window sizes.
		 * 
		 *  Next, channelCreated() will be called which will provide the SshChannel
		 *  object. This may then have event listeners attached to it to react on opening,
		 *  closing and other events.
		 */
		...
		client.addChannelHandler(new SshChannelHandler() {
			...
			public void channelCreated(final SshChannel channel) throws IOException {
				...
				channel.addListener(new SshChannelListener() {

					public void opened(SshLifecycleComponent ch) {
						SshChannel channel = (SshChannel) ch;
					...
					public void closing(SshLifecycleComponent channel) {
						System.out.println("Channel " + ((SshChannel) channel).getName() + " closed");
					}
					...
					public boolean request(SshChannel channel, String requestType, boolean wantReply, byte[] requestData) {
						System.out.println("Channel " + ((SshChannel) channel).getName() + " requests " + requestType
